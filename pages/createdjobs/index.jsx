...
const CreatedJobs = () => {
  ...
  const fetchUserJobs = useCallback(async () => {
    setLoading(true);
    try {
      const userJobs = await getUserCreatedJobs(user?.token);
      setJobs(userJobs);
      setError("");
    } catch (err) {
  ...
  const handleApply = async () => {
    ...
    try {
      await deleteCreatedJobByUser(selectedJobId, user?.token);
      handleCloseModal();
      fetchUserJobs(); // Refresh the list
      toast.success("Job deleted successfully", { position: "top-center" });
    } catch (err) {
      console.error("Delete failed:", err.message);
      toast.error(err.message || "Failed to delete job", { position: "top-center" });
    } finally {
